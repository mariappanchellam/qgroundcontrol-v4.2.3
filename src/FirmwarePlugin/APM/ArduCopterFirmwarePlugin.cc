...
{
    // DhakshaGroundControl: only Loiter, Guided, Auto, Altitude Hold, RTL and Stabilize
    // are offered in the flight mode menu, in this order. The other modes stay listed (not
    // settable) so they are still named correctly when the vehicle reports them and so the
    // Land/Brake/Smart RTL actions keep working.
    setSupportedModes({
        ...
        APMCopterMode(APMCopterMode::LAND,          false),
